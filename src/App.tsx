  ...
  const handleSubmitTriage = async () => {
    ...
    try {
      ...
      setCompletedRecord({
        ...
        patient: {
          ...newRecord.patient,
          patientId: assignedPatientId, // 차트, EMR 표기에는 순수 차트번호 사용
        },
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between text-slate-500 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-700">
              {language === 'en'
                ? 'OZ Dental Hospital Smart Reception Kiosk · Touch Screen Optimized'
                : '오즈치과대학교병원 대형 태블릿 스마트 예진 시스템 · 국민건강보험공단 EMR 실시간 연동'}
            </span>
          </div>
